...
@push('scripts')
  ...
  <script>
     ...
     //call the api for getting ongoing job requests made to the Cse
function ongoingJobsEvent(currentValue){
    ...
    var cseid = document.getElementById("currentuser").value;

          $.get( url+"/api/requests/ongoing_jobs_cse?userid="+cseid, function( data ) {
              $.each(data.data, function(key, val){
              cnt++;
              if(cnt===1){
                getInvolvedUsers(val.unique_id, cseid)
              }
              
              options ='<option>'+val.unique_id+'</option>';
...
//call the api for getting users involved with the requests made to cse( works for Cse)
function getInvolvedUsers(currentValue, cseid){
    var options="";
    var cnt;
    var url = window.location.origin;
