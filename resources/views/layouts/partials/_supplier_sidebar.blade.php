...
<aside class="aside aside-fixed">
  ...
  <div class="aside-body">
    ...
    <ul class="nav nav-aside">
      ...
      <li class="nav-item with-sub {{ Route::currentRouteNamed('supplier.rfq.warranty', 'supplier.warranty_sent_invoices') ? 'active show' : '' }}">
        <a href="" class="nav-link"><i data-feather="git-pull-request"></i> <span>Warranty Claims @if(count($RfqDispatchNotification) > 0)<span class="badge badge-primary">{{count($RfqDispatchNotification) }}</span>@endif</span></a>
        <ul>
          <li class="{{ Route::currentRouteNamed('supplier.rfq.warranty', 'supplier.rfq_send_supplier_invoice', 'supplier.rfq_link_details') ? 'active' : '' }}"><a href="{{ route('supplier.rfq.warranty', app()->getLocale()) }}">New Claims @if($warrantyQuotes > 0)<sup class="font-weight-bold text-primary">{{ $warrantyQuotes }}</sup>@endif</a></li>
