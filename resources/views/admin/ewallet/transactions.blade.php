                ...
                <table class="table table-hover mg-b-0" id="basicExample">
                    ...
                    @foreach($walletTransactions as $k=>$val)
                        <tr>
                            ...
                            <td class="tx-medium">{{ ucfirst($val['transaction_type']) }}</td>
                            <td class="tx-medium">{{ ucfirst($val['payment_type']) }}</td>
                            <td class="tx-medium text-center">{{ number_format($val['amount']) ?? 0 }}</td>
                            <td class="text-medium {{ ((is_null($val['user']['deleted_at']) ? 'text-success' : 'text-danger')) }}">{{ is_null($val['user']['deleted_at']) ? 'Active' : 'Inactive' }}</td>
                            ...
                            <td class=" text-center">
                                <div class="dropdown-file">
                                    ...
                                    <a href="#transactionDetails" id="wallet-transaction-detail" data-toggle="modal" class="dropdown-item details text-primary" title="View {{ $val['unique_id'] }} wallet transaction details" data-url="{{ route('admin.ewallet.history.details', ['history'=>$val->id, 'locale'=>app()->getLocale()]) }}"><i class="far fa-clipboard"></i> Details</a>
